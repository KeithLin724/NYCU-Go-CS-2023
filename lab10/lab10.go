package main

import (
	"bufio"
	"context"
	"fmt"
	"log"
	"net/http"
	"os"
	"strings"

	// "os"

	"github.com/gorilla/websocket"
	"github.com/reactivex/rxgo/v2"
)

type client chan<- string // an outgoing message channel

var (
	entering      = make(chan client)
	leaving       = make(chan client)
	messages      = make(chan rxgo.Item) // all incoming client messages
	ObservableMsg = rxgo.FromChannel(messages)
)

func broadcaster() {
	clients := make(map[client]bool) // all connected clients
	MessageBroadcast := ObservableMsg.Observe()
	for {
		select {
		case msg := <-MessageBroadcast:
			// Broadcast incoming message to all
			// clients' outgoing message channels.
			for cli := range clients {
				cli <- msg.V.(string)
			}

		case cli := <-entering:
			clients[cli] = true

		case cli := <-leaving:
			delete(clients, cli)
			close(cli)
		}
	}
}

func clientWriter(conn *websocket.Conn, ch <-chan string) {
	for msg := range ch {
		conn.WriteMessage(1, []byte(msg))
	}
}

func wshandle(w http.ResponseWriter, r *http.Request) {
	upgrader := &websocket.Upgrader{CheckOrigin: func(r *http.Request) bool { return true }}
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		log.Println("upgrade:", err)
		return
	}

	ch := make(chan string) // outgoing client messages
	go clientWriter(conn, ch)

	who := conn.RemoteAddr().String()
	ch <- "你是 " + who + "\n"
	messages <- rxgo.Of(who + " 來到了現場" + "\n")
	entering <- ch

	defer func() {
		log.Println("disconnect !!")
		leaving <- ch
		messages <- rxgo.Of(who + " 離開了" + "\n")
		conn.Close()
	}()

	for {
		_, msg, err := conn.ReadMessage()
		if err != nil {
			log.Println("read:", err)
			break
		}
		messages <- rxgo.Of(who + " 表示: " + string(msg))
	}
}

func ReadFile(fileName string) ([]string, error) {

	file, err := os.Open(fileName)

	if err != nil {
		return nil, err
	}
	defer file.Close()

	res := []string{}

	scanner := bufio.NewScanner(file)

	for scanner.Scan() {
		res = append(res, scanner.Text())
	}

	if err := scanner.Err(); err != nil {
		return nil, err
	}

	return res, nil
}

func InitObservable() {
	// TODO: Please create an Observable to handle the messages
	checkSwearWordArr, err := ReadFile("./swear_word.txt")
	if err != nil {
		fmt.Println(err)
		return
	}

	checkSensitiveNameArr, err := ReadFile("./sensitive_name.txt")

	if err != nil {
		fmt.Errorf("%s\n", err)
		return
	}
	/*
		ObservableMsg = ObservableMsg.Filter(...) ... {
		}).Map(...) {
			...
		})
	*/

	ObservableMsg = ObservableMsg.Filter(func(item interface{}) bool {
		for _, swearWord := range checkSwearWordArr {
			if strings.Contains(item.(string), swearWord) {
				return false
			}
		}

		return true
	}).Map(func(_ context.Context, item interface{}) (interface{}, error) {

		str := item.(string)

		for _, sensitiveName := range checkSensitiveNameArr {
			if strings.Contains(str, sensitiveName) {
				newSensitiveNameRune := []rune(sensitiveName)
				newSensitiveNameRune[1] = '*'
				newSensitiveName := string(newSensitiveNameRune)

				return strings.Replace(str, sensitiveName, newSensitiveName, -1), nil
			}
		}

		return item, nil
	})
}

func main() {
	InitObservable()
	go broadcaster()
	http.HandleFunc("/wschatroom", wshandle)

	http.Handle("/", http.FileServer(http.Dir("./static")))

	log.Println("server start at :8090")
	log.Fatal(http.ListenAndServe(":8090", nil))
}
